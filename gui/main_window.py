...
class MainWindow(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.create_entry_field(id_frame, "Badge Number:", "badge_number", 0, 0, vcmd)
        ...
        self.create_entry_field(conduta_frame, "Posologia:", "posologia", 3, 0)
        self.create_entry_field(conduta_frame, "Horário:", "horario_medicao", 3, 1)
    ...
    def create_pa_section(self, parent, r, c):
        ...
        self.entries['pa_sistolica'] = ttk.Entry(frame, width=5)
        self.entries['pa_sistolica'].pack(side="left")
        ttk.Label(frame, text="/").pack(side="left")
        self.entries['pa_diastolica'] = ttk.Entry(frame, width=5)
        self.entries['pa_diastolica'].pack(side="left")
    ...
    def create_history_section(self):
        ...
        ttk.Label(controls_frame, text="Histórico:", font=("Arial", 14, "bold")).pack(side="left")
        self.history_period_var = tk.StringVar(value=PERIODO_15_DIAS)
        combo = ttk.Combobox(controls_frame, textvariable=self.history_period_var, values=PERIODOS_FILTRO, state="readonly", width=10)
        ...
        self.history_tree.column("badge", width=80); self.history_tree.column("nome", width=150); self.history_tree.column("data_hora", width=120)
        
        scrollbar = ttk.Scrollbar(tree_frame, orient="vertical", command=self.history_tree.yview)
        self.history_tree.configure(yscrollcommand=scrollbar.set)
        self.history_tree.pack(side="left", fill="both", expand=True); scrollbar.pack(side="right", fill="y")
    ...
    def validate_integer_input(self, v): return v.isdigit() or v == ""
    ...
    def toggle_other_queixa(self, queixa):
        if queixa == "Outros":
            ...
            if not is_checked:
                utils.clear_placeholder(self.queixa_outros_entry, self.placeholders.get("queixa_outros", ""))
                utils.setup_placeholder(self.queixa_outros_entry, self.placeholders.get("queixa_outros", ""))
            else:
                utils.remove_placeholder_on_fill(self.queixa_outros_entry, self.placeholders.get("queixa_outros", ""))
    ...
    def on_badge_number_change(self, event):
        ...
        if not badge:
            ...
            fields_to_fill = {'nome': last.nome, 'login': last.login, 'gestor': last.gestor, 'turno': last.turno,
    ...
    def refresh_history_tree(self, event=None):
        ...
        days = {'15 dias': 15, '30 dias': 30, '60 dias': 60}.get(period, 15)
        if period == "YTD": days = (datetime.now() - datetime.now().replace(month=1, day=1)).days + 1
    ...
    def save_atendimento(self):
        try:
            ...
            if self.queixas_vars.get("Outros").get() and (o := self.queixa_outros_entry.get()): queixas.append(o)
            
            now = datetime.now()
            atendimento = Atendimento(
                **data,
                queixas_principais="; ".join(queixas) if queixas else "N/A",
                condutas=[Conduta(**{k.replace("conduta_",""):v for k,v in data.items() if k.startswith("conduta_")})],
    ...
    def clear_form(self, clear_badge=True):
        ...
        if self.queixa_outros_entry: self.toggle_other_queixa("Outros")

    def open_export_window(self): EditWindow(self)
